...
export default function DelivererManagement() {
  ...
  const handleAddDeliverer = async () => {
    try {
      const response = await fetch('/api/create-deliverer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ email, password, fullName, phone, isActive }),
      });

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.error || 'Failed to add deliverer');
      }

      toast.success("Deliverer added successfully");
      setIsAddDialogOpen(false);
      resetForm();
      fetchDeliverers();
    } catch (error: any) {
  ...
  const handleEditDeliverer = async () => {
    ...
    try {
      const { error } = await supabase
        .from("deliverers")
        .update({ full_name: fullName, phone, email, is_active: isActive })
        .eq("id", selectedDeliverer.id);
  ...
  const handleDeleteDeliverer = async (delivererId: string) => {
    ...
    try {
      const { error } = await supabase.from("deliverers").delete().eq("id", delivererId);
      if (error) throw error;
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Add New Deliverer</DialogTitle>
              <DialogDescription>Create a new deliverer account</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="fullName">Full Name</Label>
              ...
              <div className="grid gap-2">
                <Label htmlFor="phone">Phone</Label>
                <Input id="phone" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+251912345678" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="email">Email</Label>
                <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="deliverer@example.com" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="password">Password</Label>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Deliverers ({filteredDeliverers.length})</CardTitle>
          <CardDescription>View and manage your delivery team members</CardDescription>
        </CardHeader>
        <CardContent>
          {filteredDeliverers.length === 0 ? (
            <div className="text-center py-12">
              ...
              <TableBody>
                {filteredDeliverers.map((deliverer) => (
                  <TableRow key={deliverer.id}>
                    ...
                    <TableCell>{deliverer.phone}</TableCell>
                    <TableCell>
                      {deliverer.is_active ? <Badge variant="default">Active</Badge> : <Badge variant="secondary">Inactive</Badge>}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div className={`h-2 w-2 rounded-full ${deliverer.is_online ? 'bg-green-500' : 'bg-gray-400'}`} />
                    ...
                    <TableCell className="text-right">
                      <Button variant="ghost" size="icon" onClick={() => openEditDialog(deliverer)}>
                        <Edit className="h-4 w-4" />
                      </Button>
                      {userRole === "admin" && (
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteDeliverer(deliverer.id)}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      )}
                    </TableCell>
